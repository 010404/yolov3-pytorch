...
class weightedFeatureFusion(nn.Module):  # weighted sum of 2 or more layers https://arxiv.org/abs/1911.09070
    def __init__(self, layers, weight=False):
        ...
        self.layers = layers        # 层指标
        self.weight = weight        # 应用权重布尔
        self.n = len(layers) + 1    # 层的数量
        if weight:
            self.w = torch.nn.Parameter(torch.zeros(self.n))  # 神经层权重
...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p, img_size, var=None):
        ...
        if self.training:
            ...
            if 'default' in self.arc:  # 分离obj和cls
                torch.sigmoid_(io[..., 4:])
...
def load_darknet_weights(self, weights, cutoff=-1):
    ...
    # 建立达标 (load layers between 0 and cutoff. if cutoff = -1 all are loaded)
...
def convert(cfg='cfg/yolov3-spp.cfg', weights='weights/yolov3-spp.weights'):
    # 在每个扩展之间的PyTorch和Darknet格式转换 (i.e. *.weights convert to *.pt and vice versa)
