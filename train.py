...
import test                  # 导入 test.py 以便每个训练批次获得mAP
from models import *
...
try:  # Mixed precision training https://github.com/NVIDIA/apex
    ...
    mixed_precision = False  # 未安装

wdir = 'weights' + os.sep  # 权重dir
last = wdir + 'last.pt'
best = wdir + 'best.pt'
results_file = 'results.txt'

#--------------------------------------------------------------------------------------------------------------
# Hyperparameters (results68: 59.9 mAP@0.5 yolov3-spp-416)
...
# 覆盖超参数with hyp*.txt (optional)   hyp是超参数英文的缩写
f = glob.glob('hyp*.txt')
if f:
    print('Using %s' % f[0])
...
#-------------------------------------------------------------------------------------------------------------------
    ...
    if weights.endswith('.pt'):  # pytorch 格式     (加载预训练权重的类型，意思是pytorch产生的.pt格式)
    ...
    elif len(weights) > 0:  # darknet 格式          (加载预训练权重的类型,darknet产生的.weights格式)
        # possible weights are '*.weights', 'yolov3-tiny.conv.15',  'darknet53.conv.74' etc.
        load_darknet_weights(model, weights)      #(调用加载权重的函数)
#----------------------------------------------------------------------------------------------------------
    ...
    if device.type != 'cpu' and torch.cuda.device_count() > 1 and torch.distributed.is_available():
        dist.init_process_group(backend='nccl',         # '分布式的后端'
                                init_method='tcp://127.0.0.1:9999',  # 分布式训练init方法
                                world_size=1,        # 用于分布式训练的节点数
                                rank=0)         # 分布式训练节点秩
        model = torch.nn.parallel.DistributedDataParallel(model, find_unused_parameters=True)
        model.yolo_layers = model.module.yolo_layers  # 把yolo层调到最高水平指数
    ...
    model.nc = nc             #将类的数量附加到模型上
    model.arc = opt.arc       # 附加yolo架构
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        #更新图片权重 weights (可选择的)(optional)
        if dataset.image_weights:
            w = model.class_weights.cpu().numpy() * (1 - maps) ** 2  # class weights
            image_weights = labels_to_image_weights(dataset.labels, nc=nc, class_weights=w)
            dataset.indices = random.choices(range(dataset.n), weights=image_weights, k=dataset.n)  # rand weighted idx

        mloss = torch.zeros(4).to(device)  #平均损失losses
        ...
        for i, (imgs, targets, paths, _) in pbar:
            ...
            #批量打印结果
            mloss = (mloss * i + loss_items) / (i + 1)       #更新意味着损失
...
if __name__ == '__main__':
    ...
    if not opt.evolve:  # Train normally
        ...
        for _ in range(1):  #每一代的发展
            if os.path.exists('evolve.txt'):        # if evolve.txt exists:选择最好的hyps和突变
                # Select parent(s)    挑选父类
                parent = 'single'  #父类选择方法: 'single' or 'weighted'
                x = np.loadtxt('evolve.txt', ndmin=2)
                n = min(5, len(x))           #之前考虑突变的结果最多
                x = x[np.argsort(-fitness(x))][:n]          #前n突变
                ...
                #产生突变
                method, mp, s = 3, 0.9, 0.2           #方法，变异概率，sigma
                npr = np.random
