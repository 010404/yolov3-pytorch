...
def detect(save_img=False):
    ...
    if ONNX_EXPORT:
        ...
        onnx.checker.check_model(model)  # 检查IR是否形成良好
        print(onnx.helper.printable_graph(model.graph))  # 打印人类可读的图形表示
        return

    # 半精度
    half = half and device.type != 'cpu'      # 一半精度仅支持CUDA
    if half:
        model.half()
    ...
    if webcam:
        view_img = True
        torch.backends.cudnn.benchmark = True              # 设置为真可以加速常数图像大小推断
                ...
                if dataset.mode == 'images':
                    ...
                    if vid_path != save_path:  # new video
                        ...
                        if isinstance(vid_writer, cv2.VideoWriter):
                            vid_writer.release()  # 释放以前的视频writer
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='weights/best.pt', help='weights path')  #导入自己训练得到的权重
